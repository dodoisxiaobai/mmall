...
public class UserController {
    ...
    public ServerResponse<User> login(String username, String password, HttpSession session, HttpServletResponse httpServletResponse) {
        ...
        if (response.isSuccess()) {
            ...
            RedisPoolUtil.setEx(session.getId(), JsonUtil.obj2String(response.getData()), Const.RedisCache.REDIS_SESSION_EXTIME);
        }
        return response;
    }
    ...
    @RequestMapping(value = "forget_answer.do", method = RequestMethod.POST)
    @ResponseBody
    ...
    public ServerResponse<User> getInfomation(HttpSession session) {
        ...
        if (currentUser == null) {
            return ServerResponse.createByErrorCodeMsg(ResponseCode.NEED_LOGIN.getCode(), "用户信息不存在，errorCode=10");
        }
